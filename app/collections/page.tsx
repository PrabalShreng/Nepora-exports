...
export const metadata: Metadata = {
  ...
  description:
    "Explore Nepora Exports Pashmina shawls, wraps, and scarves through a visual collection of handcrafted pieces.",
...
export default async function CollectionsPage({
  ...
  const filtered = allProducts.filter(
    (piece) =>
      ...
      (!query ||
        `${piece.name} ${piece.material} ${piece.category}`
          .toLowerCase()
          .includes(query)),
  );
  ...
  return (
    <>
      <Header />
      <main>
        <section className="shop-heading section-pad">
          <div className="container">
            <span className="eyebrow">THE NEPORA EXPORTS COLLECTION</span>
            <h1>Pieces to keep.</h1>
            <p>
              Discover layers shaped by texture, detail, and a love for the
              handmade.
            </p>
          </div>
        </section>
        <section className="shop-content section-pad container">
          <div className="shop-toolbar">
            ...
            <form action="/collections" method="get">
              ...
              <select id="collection-sort" name="sort" defaultValue={sort}>
          ...
          {filtered.length ? (
            <div className="shop-grid">
              {filtered.map((piece) => (
                <article className="shop-card" key={piece.id}>
                  <Link href={piece.href} className="image-frame">
                    <Image
                      src={piece.image}
                      alt={piece.alt}
                      fill
